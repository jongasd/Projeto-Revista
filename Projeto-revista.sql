...
-- noticia aponta para categoria (sentido correto)
CREATE TABLE
    IF NOT EXISTS noticia (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        CONSTRAINT fk_noticia_imagem FOREIGN KEY (imagem_id) REFERENCES imagem (id) ON DELETE SET NULL
    );
...
-- categoria deve ser INDEPENDENTE, sem FK para noticia
CREATE TABLE
...
CREATE TABLE
    IF NOT EXISTS curtida (
        id INT AUTO_INCREMENT PRIMARY KEY,
        noticia_id INT NOT NULL,
        usuario_id INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_curtida_noticia FOREIGN KEY (noticia_id) REFERENCES noticia (id) ON DELETE CASCADE,
        CONSTRAINT fk_curtida_usuario FOREIGN KEY (usuario_id) REFERENCES usuario (id) ON DELETE CASCADE
    );

CREATE TABLE
    IF NOT EXISTS favoritar (
        id INT AUTO_INCREMENT PRIMARY KEY,
        noticia_id INT NOT NULL,
        usuario_id INT NOT NULL,
        created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT fk_favoritar_noticia FOREIGN KEY (noticia_id) REFERENCES noticia (id) ON DELETE CASCADE,
        CONSTRAINT fk_favoritar_usuario FOREIGN KEY (usuario_id) REFERENCES usuario (id) ON DELETE CASCADE
    );

CREATE TABLE
    IF NOT EXISTS imagem (id INT AUTO_INCREMENT PRIMARY KEY, link text NULL);